...
const getMinAge = (data) => {
  ages = data
    .filter((p) => p.fields.hasOwnProperty("age"))
    .map((p) => p.fields.age);
  return Math.min(...ages.filter((age) => age !== null));
};
...
const getMaxAge = (data) => {
  const ages = data
    .filter((p) => p.fields.hasOwnProperty("age"))
    .map((p) => p.fields.age);
  return Math.max(...ages.filter((age) => age !== null));
};
...
const getMinFare = (data) => {
  const fares = data
    .filter(
      (p) => p.fields.hasOwnProperty("fare") && typeof p.fields.fare === "float"
    )
    .map((p) => p.fields.fare);
...
const getMaxFare = (data) => {
  const fares = data
    .filter((p) => p.fields.hasOwnProperty("fare"))
    .map((p) => p.fields.fare);
  return Math.max(...fares.filter((fare) => fare !== null));
};
...
const getSurvivorsByGender = (data, gender) => {
  return data.filter(
    (p) => p.fields.survived === "Yes" && p.fields.sex === gender
  ).length;
};
...
const getCasualitiesByGender = (data, gender) => {
  return data.filter(
    (p) => p.fields.survived === "No" && p.fields.sex === gender
  ).length;
};
...
const getTotalFare = (data) => {
  const fares = data
    .filter((p) => p.fields.hasOwnProperty("fare"))
    .map((p) => p.fields.fare);
  return fares.reduce((acc, val) => acc + val, 0);
};
...
const getAverageFare = (data) => {
  const totalFare = getTotalFare(data);
  const totalPassengers = getTotalPassengers(data);
  return totalFare / totalPassengers;
};
...
const getAverageAge = (data) => {
  ...
  const totalPassengers = ages.length;

  return totalAges / totalPassengers;
};
...
const getAverageAgeByGender = (data, gender) => {
  const ages = data
    .filter((p) => p.fields.hasOwnProperty("age") && p.fields.sex === gender)
    .map((p) => p.fields.age);
  const totalAges = ages.reduce((acc, val) => acc + val, 0);
